# -*- coding: utf-8 -*-
"""
Created on Wed Dec 21 10:57:38 2022
...
def main(filename):
    ...
    # get image names
    for filename in os.listdir("C:/PhD/Objective 1/bed/JPEGImages"):
    ...
    for img in imgnames:
        annote = "C:/PhD/Objective 1/bed/Annotations/" + img + '.xml'
    ...
    create_folder("C:/PhD/Objective 1/ImageSets/Main/")
    with open("C:/PhD/Objective 1/ImageSets/Main/train.txt", 'w') as outfile:
        for name in train_list:
            outfile.write(name + "\n")
    with open("C:/PhD/Objective 1/ImageSets/Main/val.txt", 'w') as outfile:
        for name in val_list:
            outfile.write(name + "\n")
    with open("C:/PhD/Objective 1/ImageSets/Main/trainval.txt", 'w') as outfile:
        for name in train_list:
            outfile.write(name + "\n")
        for name in val_list:
            outfile.write(name + "\n")

    with open("C:/PhD/Objective 1/ImageSets/Main/test.txt", 'w') as outfile:
        for name in test_list:
            outfile.write(name + "\n")

    # create the individiual files for each label
    for label in labels:
        with open("C:/PhD/Objective 1/ImageSets/Main/"+ label +"_train.txt", 'w') as outfile:
            for name in train_list:
                if label in annotations[name]:
                    outfile.write(name + " 1\n")
                else:
                    outfile.write(name + " -1\n")
        with open("C:/PhD/Objective 1/ImageSets/Main/"+ label +"_val.txt", 'w') as outfile:
            for name in val_list:
                if label in annotations[name]:
                    outfile.write(name + " 1\n")
                else:
                    outfile.write(name + " -1\n")
        with open("C:/PhD/Objective 1/ImageSets/Main/"+ label +"_test.txt", 'w') as outfile:
            for name in test_list:
                if label in annotations[name]:
                    outfile.write(name + " 1\n")
                else:
