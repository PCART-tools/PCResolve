...
FIXTURE = os.path.join(os.path.dirname(__file__), "fixtures", "tested_projects", "recommendation")
...
def test_sqlite3_calls(calls_by_top):
    assert "sqlite3" in calls_by_top

def test_pandas_calls(calls_by_top):
    assert "pandas" in calls_by_top

def test_pickle_calls(calls_by_top):
    assert "pickle" in calls_by_top
