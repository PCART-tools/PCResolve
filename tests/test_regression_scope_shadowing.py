...
#  These regressions lock lexical shadowing behavior.
...
def test_module_np_stays_numpy(module_symbols):
    """Module-level 'np' should remain 'numpy' with lexical scopes."""
...
def test_session_call_is_requests(calls_by_top):
    """session.get(...) should be classified as 'requests'."""
...
def test_function_use_param_does_not_pollute_requests(module_symbols):
    """Module-level 'requests' should remain import-backed."""
...
def test_comprehension_var_x_not_in_module_symbols(module_symbols):
    """Comprehension variable 'x' must not leak to module scope."""
