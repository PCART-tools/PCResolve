...
FIXTURE = os.path.join(os.path.dirname(__file__), "fixtures", "tested_projects", "TSP")
...
def test_tsplib95_calls(calls_by_top):
    assert "tsplib95" in calls_by_top

def test_time_calls(calls_by_top):
    assert "time" in calls_by_top
