...
本轮对 `b1545c2` 的值流实现建立了扩展评估矩阵，未修改分析器。
当前覆盖 **83 个入口案例、472 个检查**，其中 421 项符合预期，51 项暴露已知缺口，
分布在 21 个入口案例中。51 是失败检查数，同一问题可能影响绑定和多条值流，不能视为 51 个独立缺陷。

查看[逐项评估报告](value-flow-matrix-baseline.md)和[完整 JSON](value-flow-matrix-baseline.json)。
...
上述顺序是根据本轮误报与遗漏安排的开发建议，不表示动态类型边界均可消除。
...
`tests/value_flow_known_gaps.json` 明确列出当前 51 项缺口的检查 ID、结果类型和原因。
...
本轮完整回归记录为 **1,898 passed、51 xfailed、41 warnings**。
随后补充的评估器报告/定义时调用检查及最终专项重跑为 **447 passed、51 xfailed**。
这些 warnings 来自既有真实项目 fixture 的转义序列。另对新增 Python 文件执行了 Python 3.9 语法解析检查。
