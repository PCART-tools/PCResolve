...
| aia_fits_to_np.py:68:10 | `fn.split('_')[0].replace('AIA', '')` | python / python | python / python | builtin_method_local_receiver | static_context | v: source context: fn is a filename string yielded by os.walk; split and replace pr |
| aia_fits_to_np.py:68:10 | `fn.split('_')` | python / python | python / python | builtin | static_context | v: handle() unpacks fn from the filename tuple passed to the Pool.map callback |
| aia_fits_to_np.py:70:21 | `fn.split('_')[-1].replace('.fits', '')` | python / python | python / python | builtin_method_local_receiver | static_context | v: source context: fn is a filename string yielded by os.walk; split and replace pr |
| aia_fits_to_np.py:70:21 | `fn.split('_')` | python / python | python / python | builtin | static_context | v: handle() unpacks fn from the filename tuple passed to the Pool.map callback |
