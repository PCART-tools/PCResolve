# SDOML — static_context (18 records)
...
| aia_fits_to_np.py:68:10 | `fn.split('_')[0].replace('AIA', '')` | python / python | unknown / unknown | builtin_method_local_receiver | static_context | v: source context: fn is a filename string yielded by os.walk; split and replace pr |
| aia_fits_to_np.py:68:10 | `fn.split('_')` | python / python | unknown / unknown | builtin | static_context | v: handle() unpacks fn from the filename tuple passed to the Pool.map callback |
| aia_fits_to_np.py:70:21 | `fn.split('_')[-1].replace('.fits', '')` | python / python | unknown / unknown | builtin_method_local_receiver | static_context | v: source context: fn is a filename string yielded by os.walk; split and replace pr |
| aia_fits_to_np.py:70:21 | `fn.split('_')` | python / python | unknown / unknown | builtin | static_context | v: handle() unpacks fn from the filename tuple passed to the Pool.map callback |
...
| hmi_fits_to_np.py:83:12 | `p.replace(src, '')` | python / python | python / python | builtin_method_local_receiver | static_context | v: source context: p is a path string yielded by os.walk, so replace is a builtins. |
