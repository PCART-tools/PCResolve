...
from .container_resolution import (
    ContainerResolutionMixin, _tuple_source_item,
)
...
class ProjectAnalyzer(CallResultResolutionMixin, InstanceMethodResolutionMixin,
                      ContainerResolutionMixin, ProjectResultBindingMixin):
    # ── pipeline ───────────────────────────────────────────────────────
    ...
    def _resolve_structured_source(self, module, direct_source, tracers,
        ...
        if kind == "instance_method":
            return self._resolve_instance_method_source(
                module, direct_source, a, b, tracers, _seen)
        ...
        if kind == "container_iter":
            return self._resolve_container_iter_source(module, a, tracers)

        if kind == "call_result":
            return self._resolve_call_result_source(
                module, direct_source, a, callee_display, tracers, _seen)
